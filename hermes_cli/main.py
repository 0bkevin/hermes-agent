...
def cmd_brio(args):
    """Brio relay connector (mobile control plane)."""
    from hermes_cli.brio import brio_command

    return brio_command(args)
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        "acp", "auth", "backup", "brio", "bundles", "checkpoints", "claw", "completion",
        "computer-use",
...
def main():
    ...
    # =========================================================================
    # brio command — Brio relay connector (mobile control plane). Connects
    # Hermes to the Brio relay so the Brio mobile app can reach this agent
    # without a separate companion binary.
    # =========================================================================
    brio_parser = subparsers.add_parser(
        "brio",
        help="Brio relay connector",
        description=(
            "Connect Hermes to the Brio control plane. Enroll with a code "
            "from the Brio mobile app, then run the connector so the app "
            "can reach this agent through the Brio relay."
        ),
    )
    brio_subparsers = brio_parser.add_subparsers(dest="brio_command")

    brio_enroll = brio_subparsers.add_parser(
        "enroll",
        help="Claim an enrollment code and store relay credentials",
    )
    brio_enroll.add_argument(
        "--relay-url",
        dest="relay_url",
        required=True,
        help="Brio relay base URL (e.g. https://relay.example.com)",
    )
    brio_enroll.add_argument(
        "--code",
        required=True,
        help="Enrollment code generated by the Brio mobile app",
    )
    brio_enroll.add_argument(
        "--name",
        default=None,
        help="Agent display name (default: this machine's hostname)",
    )

    brio_subparsers.add_parser(
        "connect",
        help="Run the relay tunnel connector in the foreground",
    )

    brio_subparsers.add_parser(
        "status",
        help="Show stored configuration (redacted) and relay/tunnel health",
    )

    brio_recover = brio_subparsers.add_parser(
        "recover",
        help="Recover relay credentials with an owner device token",
    )
    brio_recover.add_argument(
        "--relay-url",
        dest="relay_url",
        required=True,
        help="Brio relay base URL",
    )
    brio_recover.add_argument(
        "--agent-id",
        dest="agent_id",
        required=True,
        help="Agent ID to recover",
    )
    brio_recover.add_argument(
        "--device-token",
        dest="device_token",
        required=True,
        help="Owner device token from the relay (see the Brio mobile app)",
    )
    brio_recover.add_argument(
        "--name",
        default=None,
        help="Optional agent display name override",
    )
    brio_parser.set_defaults(func=cmd_brio)
